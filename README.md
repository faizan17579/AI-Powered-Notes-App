...
1. **Show the login/register page** 
   - "Welcome to the AI Note-Taking App - a secure, smart way to organize your thoughts"
   - Register a new account or login
   - Show successful authentication and redirect to home page

### Creating a Note (30 seconds)
2. **Click "Add Note" button**
   - Enter title: "Project Meeting Notes"
   - Enter content: "Discussed Q4 roadmap. Key priorities: launch mobile app, improve performance by 40%, expand to European markets. Budget approved for two new hires. Next meeting scheduled for November 15th."
   - **Click "Save Note"**
   - Show redirect to home page with new note

### AI Summarization Demo (45 seconds)
3. **Click "Summarize" button** on the note
   - Show loading state ("Summarizing...")
   - Display generated summary in blue box
   - Read summary aloud: "This note discusses Q4 priorities including mobile launch, performance improvements, and market expansion, with budget approval for hiring."
   - Explain: "AI automatically extracts key points using Google Gemini"

### Search Functionality (20 seconds)
4. **Demonstrate search with debounce**
   - Start typing "meeting" in search bar slowly
   - Show the search indicator (spinning loader)
   - Results appear after you stop typing (300ms delay)
   - Show results count "1 of 2 notes matching 'meeting'"
5. **Clear search**
   - Click the X button to clear
   - All notes reappear instantly

### Creating More Notes (15 seconds)
6. **Create another note** quickly
   - Title: "Grocery List"
   - Content: "Milk, eggs, bread, coffee, bananas, yogurt"
   - Save
7. **Show search in action**
   - Search for "grocery" - finds the new note
   - Search for "coffee" - also finds it
   - Clear to show all notes again

### Editing & Deleting (30 seconds)
8. **Click "Edit" on a note**
   - Modify some content
   - Save changes
   - **Show Version History**: Click "View History" button
   - Display version history modal with all previous versions
   - **Click on a version card** to view full details
   - Show complete title, content, and summary for that version
   - **Restore a previous version** 
   - Confirm restoration
   - Show note reverted to previous state

### Export/Import Demo (20 seconds)
9. **Demonstrate Export functionality**
   - Click "Export" button on a note card
   - Select format (TXT, Markdown, or PDF)
   - Show file downloading
   - Click "Export All" to download all notes
10. **Demonstrate Import functionality**
    - Click "Import Note" button
    - Select a .txt or .md file
    - Show import preview modal with editable content
    - Save imported note
    - Note appears in the grid

### Deleting (10 seconds)
11. **Click "Delete" on a note**
   - Show confirmation modal
   - Confirm deletion
   - Note disappears from grid

### Closing (10 seconds)
12. **Show final home page** with remaining notes
    - Demonstrate responsive design (resize window if possible)
    - Toggle dark mode to show theme support
    - "Built with Next.js, MongoDB, JWT authentication, and Google Gemini AI"
    - "Features intelligent search, version history, export/import, and user authentication"
    - Display GitHub repo link

### Pro Tips for Video:
- Use browser zoom (Ctrl/Cmd + +) to make text more readable
- Enable smooth cursor tracking in screen recording software
- Prepare notes content in advance to speed up typing
- Use a note with 3-4 sentences for best summarization demo
- Show responsive design by resizing browser window at the end
